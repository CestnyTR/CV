...
function openPopupInfo(i) {
  let innerHTML = ""
  innerHTML += ` 
  <div id="popup-container">
  <iframe src="${EN_VideoProjectInfo[i-1].LinkVideo}" title="YouTube video player" allowfullscreen></iframe>
  </div> `;
...
function LightTheme() {
  ...
  document.getElementById("btnContant").style.color = "whitesmoke";
}
...
function buildlang(hamburgerMenuData, navBarData, languageData, themeData, heads, meInfoData, VideoProjectData, LinkedProjectData, talentsData, certificatesData, schoolData, experiencesData, contactsData) {
  //!NavBar
  navbarBuilder(hamburgerMenuData, navBarData, languageData, themeData);

  //!me info data
  meInfoBuild(heads, meInfoData);

  //!project data
  projectBuild(heads, VideoProjectData, LinkedProjectData);

  //!talents
  talentBuild(heads, talentsData)

  //!certificates

  certificatesBuild(heads, certificatesData)
  //!School
  schoolBuild(heads, schoolData)
  //!experiences
  experienceBuild(heads, experiencesData);

  //!contact
  contactsBuild(heads, contactsData)
}

function navbarBuilder(hamburgerMenuData, navBarData, languageData, themeData) {
...
function meInfoBuild(heads, meInfoData) {
  let meInfoHTML = ""
  ...
  <span id="btnContant">${heads[0].head}</span>
  <div class="container">
...
function projectBuild(heads, VideoProjectData, LinkedProjectData) {
  //?video
  ...
  <div class="projectLink" id="${VideoProjectData[i].projectID}" onclick="openPopupInfo(${VideoProjectData[i].id})">
...
function talentBuild(heads, talentsData) {
...
function certificatesBuild(heads, certificatesData) {
...
function schoolBuild(heads, schoolData) {
...
function experienceBuild(heads, experiencesData) {
...
function contactsBuild(heads, contactsData) {
